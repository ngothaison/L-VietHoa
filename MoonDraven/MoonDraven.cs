...
namespace MoonDraven
{
    ...
    internal class MoonDraven
    {
        ...
        private void CreateMenu()
        {
            ...
            comboMenu.AddItem(new MenuItem("UseQCombo", "Dùng Q").SetValue(true));
            comboMenu.AddItem(new MenuItem("UseWCombo", "Dùng W").SetValue(true));
            comboMenu.AddItem(new MenuItem("UseECombo", "Dùng E").SetValue(true));
            comboMenu.AddItem(new MenuItem("UseRCombo", "Dùng R").SetValue(true));
            this.Menu.AddSubMenu(comboMenu);

            // Harass
            var harassMenu = new Menu("Harass", "harass");
            harassMenu.AddItem(new MenuItem("UseEHarass", "Dùng E").SetValue(true));
            harassMenu.AddItem(
            ...
            laneClearMenu.AddItem(new MenuItem("UseQWaveClear", "Dùng Q").SetValue(true));
            laneClearMenu.AddItem(new MenuItem("UseWWaveClear", "Dùng W").SetValue(true));
            laneClearMenu.AddItem(new MenuItem("UseEWaveClear", "Dùng E").SetValue(false));
            laneClearMenu.AddItem(new MenuItem("WaveClearManaPercent", "Phần trăm mana").SetValue(new Slider(50)));
            this.Menu.AddSubMenu(laneClearMenu);

            // Axe Menu
            var axeMenu = new Menu("Cài đặt rìu", "axeSetting");
            axeMenu.AddItem(
                new MenuItem("AxeMode", "bắt rìu với chế độ:").SetValue(
                    new StringList(new[] { "Combo", "Bất kỳ", "Luôn luôn" }, 2)));
            axeMenu.AddItem(new MenuItem("CatchAxeRange", "Tầm bắt rìu").SetValue(new Slider(800, 120, 1500)));
            axeMenu.AddItem(new MenuItem("MaxAxes", "Rìu tối đa").SetValue(new Slider(2, 1, 3)));
            axeMenu.AddItem(new MenuItem("UseWForQ", "Dùng W nếu rìu quá xa").SetValue(true));
            axeMenu.AddItem(new MenuItem("DontCatchUnderTurret", "Không bắt rìu khi trong trụ").SetValue(true));
            this.Menu.AddSubMenu(axeMenu);

            // Drawing
            var drawMenu = new Menu("Drawing", "draw");
            drawMenu.AddItem(new MenuItem("DrawE", "Vẽ tầm E").SetValue(true));
            drawMenu.AddItem(new MenuItem("DrawAxeLocation", "vẽ vị trí bắt rìu").SetValue(true));
            drawMenu.AddItem(new MenuItem("DrawAxeRange", "vẽ tầm bắt rìu").SetValue(true));
            this.Menu.AddSubMenu(drawMenu);

            // Misc Menu
            var miscMenu = new Menu("Misc", "misc");
            miscMenu.AddItem(new MenuItem("UseWSetting", "Dùng W ngay lập tức(Khi có sẵn)").SetValue(false));
            miscMenu.AddItem(new MenuItem("UseEGapcloser", "Dùng E on Gapcloser").SetValue(true));
            miscMenu.AddItem(new MenuItem("UseEInterrupt", "Dùng E để ngắt chiêu").SetValue(true));
            miscMenu.AddItem(new MenuItem("UseWManaPercent", "Phần trăm mana dùng W").SetValue(new Slider(50)));
            miscMenu.AddItem(new MenuItem("UseWSlow", "Dùng W nếu bị làm chậm").SetValue(true));
